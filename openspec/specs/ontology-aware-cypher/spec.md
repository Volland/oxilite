...
When shape checks are enabled, the system SHALL check the shape constraints covering every node a Cypher write creates or changes, on the node's final state, before the write request is sent. The checked constraints are `sh:datatype`, `sh:minCount`, `sh:maxCount`, `sh:in` and `sh:pattern`. A violation MUST abort the statement with no change applied.
