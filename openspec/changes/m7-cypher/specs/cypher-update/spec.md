...
The system SHALL execute each Cypher statement that writes as one atomic request, so a failure leaves the store unchanged. The request evaluates its reading part before any modification, as SPARQL UPDATE does.
...
The system SHALL give each node created by `CREATE` or `MERGE` a fresh id, computed without reading from the store, even when many nodes are created per matched row. This MUST work on backends without UDFs.
