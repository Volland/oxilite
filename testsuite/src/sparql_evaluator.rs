...
fn evaluate_evaluation_test(test: &Test) -> Result<()> {
    ...
    // oxilite port: GeoSPARQL extension functions (spargeo) are not registered.
    ...
    // Verdict 2 (upstream sanity): Oxigraph's evaluator against the expected results.
    ...
    for with_query_optimizer in [true, false] {
        ...
        if !are_query_results_isomorphic(&expected_results, &actual_results) {
            upstream = Err(anyhow::anyhow!(
                "upstream: Oxigraph results are not isomorphic.\n{}\nParsed query:\n{query}\nData:\n{dataset}\n",
                results_diff(load_sparql_query_result(test.result.as_ref().unwrap())?, actual_results),
            ));
            break;
        }
    }

    // oxilite port: verdicts 1 (oxilite vs expected) and 3 (oxilite vs Oxigraph) for every
    // configured oxilite variant.
    if !test.service_data.is_empty() || crate::engine::uses_service(&query) {
        return upstream; // SERVICE is out of scope for oxilite
    }
    let oxigraph_results = StaticQueryResults::from_query_results(
        evaluator.prepare(&query).execute(&dataset)?,
        with_order,
    )?;
    for variant in crate::engine::variants() {
        ...
        let actual = StaticQueryResults::from_query_results(actual, with_order)
            ...
            are_query_results_isomorphic(&expected_results, &actual),
            "oxilite[{variant}]: results are not isomorphic with the expected results.\n{}\nParsed query:\n{query}\n{explain}\nData:\n{dataset}\n",
            results_diff(load_sparql_query_result(test.result.as_ref().unwrap())?, actual),
        );
        if upstream.is_ok() {
            ensure!(
                are_query_results_isomorphic(&oxigraph_results, &actual),
                "oxilite[{variant}] and Oxigraph disagree.\nParsed query:\n{query}\n{explain}\n"
            );
        }
    }
    upstream.map_err(|e| anyhow::anyhow!("{e}\n(oxilite passes this test)"))
}
...
fn evaluate_update_evaluation_test(test: &Test) -> Result<()> {
    ...
    let initial: Dataset = store.iter().collect::<Result<_, _>>()?;
    store
        .update(update.clone())
    ...
    let upstream = if store_dataset == result_store_dataset {
        Ok(())
    } else {
        Err(anyhow::anyhow!(
            "upstream: Not isomorphic result dataset.\nDiff:\n{}\nParsed update:\n{}\n",
    ...
    for variant in crate::engine::variants() {
        ...
        actual.canonicalize(CanonicalizationAlgorithm::Unstable);
            ...
            actual == result_store_dataset,
            "oxilite[{variant}]: not isomorphic result dataset.\nDiff:\n{}\nParsed update:\n{}\n",
            dataset_diff(&result_store_dataset, &actual),
            update
        );
    }
    upstream
