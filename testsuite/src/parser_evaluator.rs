...
fn evaluate_eval_test(
    ...
    // oxilite port: the parsed dataset must round-trip through every oxilite store variant.
    for variant in crate::engine::variants() {
        let engine = crate::engine::OxiliteEngine::new(variant, &actual_dataset)?;
        let mut stored = engine.dataset()?;
        stored.canonicalize(CanonicalizationAlgorithm::Unstable);
        ensure!(
            stored == actual_dataset,
            "oxilite[{variant}]: store round-trip changed the dataset. Diff:\n{}",
            dataset_diff(&actual_dataset, &stored)
        );
    }
    Ok(())
}
